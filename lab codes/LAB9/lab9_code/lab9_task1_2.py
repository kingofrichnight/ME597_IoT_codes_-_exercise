...
# RMS threshold for AFF running detection
X_RMS_THRESHOLD = 1.95   # m/s^2
...
while True:
    try:
        ...
        # Determine AFF execution state using X-axis
        if x_rms > X_RMS_THRESHOLD:
            state = "ACTIVE (RUNNING)"
        else:
            state = "STOPPED"

        print("X-axis rms={0:.4f} m/s2, Y-axis rms={1:.4f} m/s2, Z-axis rms={2:.4f} m/s2 --> AFF state: {3}".format(
            x_rms, y_rms, z_rms, state
        ))
