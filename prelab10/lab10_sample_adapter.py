...
CHUNK = int(2048) # chunk size
AGENT = "http://192.168.1.4:5001/" # MTConnect agent ip:port

N = int(23) # number of sequence to take sound
N_FFT = int(2048) # number of FFT
N_MELS = int(128) # number of Mel filter bank

model_file = "20260407_182820_Prelab10_CNN_model1.h5"  # CNN model fileanme. Note that this file must be in the same directory
    ...
    def adapter_stream(self):
        while True:
            try:
                ...
                x = get_sound_signal(requests.get(AGENT+SAMPLE+"?from="+str(int(lastSeq-N+1))+"&count="+str(N)+"&path=//DataItem[@id=%27sensor1%27]")) # request MTConnect sound streams as many as we need
